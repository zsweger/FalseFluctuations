...
int main(int argc, char* argv[])
{
    ...
    for(unsigned int iLine=0; iLine<vec.size(); ++iLine){
        ...
        //Int_t np[6][12] = {0};
        Int_t np[27] = {0};
        TVectorD *pileupRate_pointer = (TVectorD*)inf->Get("pileupFraction");
        pileupRate = pileupRate_pointer->operator()(0);
    ...
    for(int cbin=0; cbin<7; ++cbin) {
        ...
        StartBin = Cent[cbin];
        if(cbin == 0)EndBin=EndBinAll[DATASETINDEX];
    ...
    cout<<"most central C4/C2 = "<<OutGraph[8]->GetY()[0]<<" +/- "<<OutGraph[8]->GetErrorY(0)<<endl;
    
    if(DEBUG) cout<<"Writing .."<<endl;
    TString outName = "dummy.root";
    outName = Form("Output/cum_%d_%fpileupRate.root", INDEX1,pileupRate);
