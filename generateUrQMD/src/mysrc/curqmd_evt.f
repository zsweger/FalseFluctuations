...
c     TODO load a unique seed for every event and for every job
      ...
      if (c_use_external_seed) then
        ranseed = c_external_seed
      endif

      if (c_quiet .eq. 0) then
        write(6,*)'event# ',event,ranseed
      endif
      c_current_event_seed = ranseed
...
c final output
         ...
         if(ctag.eq.0)then
           ...
           if (c_skip_empty_event .eq. 1) then
             c_current_is_empty = 1
c            rest this event
             mp = old_mp
             mc = old_mc
           else
...
c   calculate number of participant particles
c   Npart = Nucleons from Projector and Target - zero inelastic collisions 
         cnspec = 0
         do i = 1, npart
           if (nincoll(i) .eq. 0) cnspec = cnspec + 1
         end do
         cnpartit = Ap + At - cnspec
         cmul = npart

      end subroutine ! curqmd_evt
